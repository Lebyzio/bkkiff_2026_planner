...
- **ดาวน์โหลดแผนเป็นปฏิทิน (.ics)** นำเข้า Google/Apple Calendar ได้โดยตรง (เวลาแปลงจาก Bangkok ไป UTC ให้อัตโนมัติ)
- **คัดลอกแผนเป็นข้อความ** — สรุปรอบฉายทั้งหมด (วัน/เวลา/ชื่อหนัง/โรง/หมายเหตุ) เป็นข้อความอ่านง่าย คัดลอกไปวางแชทหาเพื่อนได้ทันที ไม่ต้องกดลิงก์
...
- `src/lib/exportPlanIcs.ts` — builds the "My Plan" `.ics` file (pure string building, no library needed); `buildPlanIcs` is unit-tested in `src/lib/exportPlanIcs.test.ts`, `downloadPlanIcs` wraps it with the Blob/anchor download dance.
- `src/lib/exportPlanText.ts` — builds the plain-text plan summary used by the "copy as text" button; `buildPlanText` is unit-tested in `src/lib/exportPlanText.test.ts`.
